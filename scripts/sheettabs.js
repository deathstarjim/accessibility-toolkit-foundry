...
function applyInventoryAccessibility(root)
{
    ...
    const controls = root.querySelectorAll(
        ".item-name, .item-image, .rollable, .tidy-table-row-use-button, .item-toggle, .command.decrementer, .command.incrementer, .tidy-table-button, .button.button-icon-only, .quantity-tracker-input"
    );
    ...
    for (const control of controls)
    {
        ...
        if (
            (control.matches(".item-name, .item-image, .tidy-table-row-use-button, .item-toggle, .command.decrementer, .command.incrementer, .tidy-table-button, .button.button-icon-only")
                || control.matches(".item-image, .rollable")
...
function attachSheetTabHandlers(root, app)
{
    ...
    root.addEventListener("keydown", event =>
    {
        ...
        const actionable = isInventoryKeyboardActionTarget(target);
        ...
        debugSheetTabs("inventory keyboard action activated", {
            ...
            targetClasses: resolvedTarget.className,
    ...
    root.addEventListener("click", event =>
    {
        ...
        event.preventDefault();
        event.stopPropagation();
        void activateInventoryControl(activationTarget, app, null);
    }, true);
}
