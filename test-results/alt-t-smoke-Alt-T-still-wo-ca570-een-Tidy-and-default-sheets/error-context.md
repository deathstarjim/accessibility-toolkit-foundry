...
- Name: alt-t-smoke.spec.js >> Alt+T still works after changing the actor between Tidy and default sheets
- Location: tests\playwright\alt-t-smoke.spec.js:168:1
...
Locator: locator('.application.sheet.actor.character[id$="Actor-EKXtdJ94pp1DLGIJ"]').filter({ visible: true }).last().locator('.controls-dropdown:visible')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.application.sheet.actor.character[id$="Actor-EKXtdJ94pp1DLGIJ"]').filter({ visible: true }).last().locator('.controls-dropdown:visible')
...
- generic [active] [ref=e1]:
      ...
      - generic:
        ...
        - complementary:
          - generic [ref=e16]:
            ...
            - generic [ref=e22]:
              - generic [ref=e23]: Latency 22ms
              - generic [ref=e24]: FPS 28
  ...
  - generic [ref=e1370]:
    - banner [ref=e1371]:
      - generic [ref=e1372]: 
      - 'heading "Player Character: Sheet Configuration" [level=1] [ref=e1373]'
      - text: 
      - button "Copy Document UUID" [ref=e1374] [cursor=pointer]: 
      - button "Close Window" [ref=e1375] [cursor=pointer]: 
    - generic [ref=e1376]:
      - generic [ref=e1377]:
        - group "Document" [ref=e1378]:
          - generic [ref=e1379]: Document
          - generic [ref=e1380]:
            - generic [ref=e1381]: This Sheet
            - combobox [ref=e1383] [cursor=pointer]:
              - option "Default Sheet"
              - option "D&D 5e Character Sheet" [selected]
              - option "Tidy 5e Character Sheet"
              - option "Tidy 5e Classic Character Sheet"
            - paragraph [ref=e1384]: Override which sheet type you would like to use for this specific document.
          - generic [ref=e1385]:
            - generic [ref=e1386]: Theme
            - combobox [ref=e1388] [cursor=pointer]:
              - option "Default" [selected]
              - option "Dark"
              - option "Light"
            - paragraph [ref=e1389]: Not all themes are available for each sheet.
        - group "Defaults" [ref=e1390]:
          - generic [ref=e1391]: Defaults
          - generic [ref=e1392]:
            - generic [ref=e1393]: Default Sheet
            - combobox [disabled] [ref=e1395]:
              - option "D&D 5e Character Sheet" [selected]
              - option "Tidy 5e Character Sheet"
              - option "Tidy 5e Classic Character Sheet"
            - paragraph [ref=e1396]: Set the default sheet type used for this document type (GM Only).
          - generic [ref=e1397]:
            - generic [ref=e1398]: Theme
            - combobox [ref=e1400] [cursor=pointer]:
              - option "Default" [selected]
              - option "Dark"
              - option "Light"
            - paragraph [ref=e1401]: Not all themes are available for each sheet.
      - button " Save Sheet Configuration" [ref=e1403] [cursor=pointer]:
        - generic: 
        - generic: Save Sheet Configuration
```

# Test source

```ts
  27  |     await userSelect.selectOption({ label: "Tester the Brave" });
  28  |     const joinButton = joinForm.locator('button[name="join"]');
  29  |     await expect(joinButton).toBeVisible();
  30  |     await joinButton.click();
  31  |     await page.waitForURL("**/game");
  32  |     await page.waitForFunction(() => Boolean(game.ready), null, { timeout: 15000 });
  33  | }
  34  | 
  35  | async function logUserCharacterDiagnostics(page, actorName = "Tester the Brave")
  36  | {
  37  |     const diagnostics = await page.evaluate((name) =>
  38  |     {
  39  |         const user = game.user;
  40  |         const assignedCharacter = user?.character ?? null;
  41  |         const namedActor = game.actors?.find(candidate => candidate?.type === "character" && candidate?.name === name) ?? null;
  42  |         const namedActorOwnership = namedActor && user ? namedActor.testUserPermission(user, "OWNER") : false;
  43  | 
  44  |         return {
  45  |             userName: user?.name ?? null,
  46  |             assignedCharacterName: assignedCharacter?.name ?? null,
